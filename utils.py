...
from settings import PROJECT_MAX_PORT, PROJECT_MIN_PORT, PROJECTS_DIR, CONFIG_FILEPATH
...
# Use the already defined CONFIG_FILEPATH from settings.py
# CONFIG_FILEPATH is now imported from settings.py

# Define the default configuration with all settings
DEFAULT_CONFIG = {
    "credentials": {
        "civitai": {
            "apikey": ""
...
def get_config():
    """
    Get the configuration from config.json, creating it with default values if it doesn't exist.
    ...
    try:
        ...
        if os.path.exists(CONFIG_FILEPATH):
            ...
            # If file doesn't exist, create with defaults and return defaults
            with open(CONFIG_FILEPATH, "w") as f:
                json.dump(DEFAULT_CONFIG, f, indent=2)
            return DEFAULT_CONFIG.copy()
    except (FileNotFoundError, json.JSONDecodeError, PermissionError) as e:
        print(f"Warning: Could not load config file: {e}")
        # Create with defaults as fallback
        try:
            with open(CONFIG_FILEPATH, "w") as f:
                json.dump(DEFAULT_CONFIG, f, indent=2)
...
def update_config(config_update):
    """
    Update the configuration, merging with existing values
    
    Args:
        config_update (dict): Dictionary containing configuration updates
    ...
    # Deep merge the update with existing config
    ...
    deep_merge(config, config_update)
    set_config(config)
    return config
